...
// Error represents an error in a Hrpc service call.
type Error interface {
	...
	// Error returns a string of the form "hrpc error <Type>: <Msg>"
	Error() string
}

// WrapError allows Hrpc errors to wrap other errors.
...
// NewError is the generic constructor for a hrpc.Error. The ErrorCode must be
...
// This function is used by Hrpc services to wrap non-Hrpc errors as internal errors.
// The wrapped error can be extracted later with (github.com/pkg/errors).Unwrap
// or errors.Is from the standard errors package on Go 1.13+.
...
// ErrorCode represents a Hrpc error type.
type ErrorCode string

// Valid Hrpc error types. Most error types are equivalent to gRPC status codes
...
const (
	...
	// BadRoute means that the requested URL path wasn't routable to a Hrpc
...
// ServerHTTPStatusFromErrorCode maps a Hrpc error type into a similar HTTP
// response status. It is used by the Hrpc server handler to set the HTTP
...
func (e *twerr) Error() string {
	return fmt.Sprintf("hrpc error %s: %s", e.code, e.msg)
}

// wrappedErr is the error returned by hrpc.InternalErrorWith(err), which is used by clients.
...
// WriteError writes an HTTP response with a valid Hrpc error format (code, msg, meta).
// Useful outside of the Hrpc server (e.g. http middleware).
// If err is not a hrpc.Error, it will get wrapped with hrpc.InternalErrorWith(err)
...
// marshalErrorToJSON returns JSON from a hrpc.Error, that can be used as HTTP error response body.
